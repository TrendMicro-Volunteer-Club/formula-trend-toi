...
class WebSocket(object):
   ...
   def sendMessage(self, data, force_text = False):
      """
          ...
          If the data is a bytearray object then the frame is sent as Binary.
      """
      opcode = TEXT if force_text else BINARY
      if _check_unicode(data):
         opcode = TEXT
...
class SimpleWebSocketServer(object):
   def __init__(self, host, port, websocketclass, user_context = None, selectInterval = 0.001):
   ...
   def _constructWebSocket(self, sock, address):
      return self.websocketclass(self, sock, address, self.user_context)
   ...
   def serve_receiving_once(self):
      if self.selectInterval:
         rList, _, xList = select(self.listeners, [], self.listeners, self.selectInterval)
      else:
         rList, _, xList = select(self.listeners, [], self.listeners)
   ...
   def serve_sending_once(self):
      ...
      if self.selectInterval:
         _, wList, _ = select([], writers, [], self.selectInterval)
      else:
         _, wList, _ = select([], writers, [])
   ...
   def sendFragmentStart(self, data):
      for client in self.connections.values():
          client.sendFragmentStart(data)

   def sendFragment(self, data):
      for client in self.connections.values():
          client.sendFragmentEnd(data)
...
class SimpleSSLWebSocketServer(SimpleWebSocketServer):
   ...
   def __init__(self, host, port, websocketclass, certfile,
                keyfile, version = ssl.PROTOCOL_TLSv1, user_context = None, selectInterval = 0.001):

      SimpleWebSocketServer.__init__(self, host, port,
                                        websocketclass, user_context, selectInterval)
